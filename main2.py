...
def reverse_seat() :
    return driver.find_elements(By.CSS_SELECTOR, '#TmgsTable > tbody > tr > td > img:nth-child(even))')[-1]
...
def safety_captcha() :
    while(True) :
        ...
        if class_name("capchaBtns").is_displayed() == True:
            ...
            answer = solve_captcha(id('imgCaptcha').screenshot_as_png, 2)
...
def 좌석선택() :
    ...
    safety(lambda : reverse_seat().click())
    # 좌석선택 버튼
    safety(lambda : driver.switch_to.default_content())
    safety(lambda : driver.switch_to.frame(name('ifrmSeat')))
